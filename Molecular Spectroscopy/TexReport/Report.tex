...
\subsection{\label{subsec:level20} $\text{CO}_{2}$}
...
\begin{figure}
    \includegraphics[width=\linewidth]{Images/LM_CO-CO2_Spectrum.png} 
    ...
    \label{fig:COCO2LM}
\end{figure}
Note the bumpy area from $3500 \text{cm}^{-1}$ to $4000 \text{cm}^{-1}$, $\text{H}_{2}\text{O}$ and $\text{CO}_{2}$ have been identified in lower wavenumbers and this is a mix of the two \cite{FTIRSPECTRAOFSMOKE}.
...
\begin{table}[h]
    \begin{tabular}{ |p{1.9cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|  }
     \hline
     \multicolumn{5}{|c|}{Area Under Identifying Regions For Each Cigar Brand In $\text{cm}^{-1}$ Units} \\ \hline
...
\end{table}
  ...
  Every substance was identified based upon its characteristic absorption lines using papers cited in the references. Some regions contain multiple absorption lines that overlap, thus restricting the ability to identify the substances that make them up, such as the region of 1300-2000$cm^{-1}$ that contains water but also many other substances such as acetone, acetaldehyde and acrolein \cite{FTIRSPECTRAOFSMOKE}, which is why acetaldehyde total absorption could not be estimated. Although for NEXT and Marlborro cigars the IR spectrum was limited, and only CO and $\text{CO}_{2}$ absorption lines were identified (see Figure \ref{fig:COCO2NEXT}), a comparison between the different cigars based upon these gasses could still be made. Since there are absorption lines of $\text{CO}_{2}$ outside the region of $1999 \text{cm}^{-1}$ to $2402 \text{cm}^{-1}$ yet they are missing from the NEXT and Marlborro cigars IR spectrums the ratios of total absorbance between cigars for these gasses, which according to \ref{eq:BeerLambert} is also the ratio between these gasses concentrations, was calculated only using said region.  

  \begin{figure}
    \includegraphics[width=\linewidth]{Images/NEXT_CO-CO2_Spectrum.png} 
